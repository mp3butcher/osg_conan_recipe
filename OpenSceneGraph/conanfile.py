...
class OpenSceneGraphConan(ConanFile):
    ...
    def source(self):       
	#win32 depend stuff ( perhaps should be put in system_requirements but i doubt there's a working dir in this hook)
    ...
    def build(self):
        ...
        # Explicit way:
        # self.run('cmake %s/OpenSceneGraph %s' % (self.source_folder, cmake.command_line))
    ...
    def customDependenciesDownload(self):
        if  self.settings.os == "Windows" and self.settings.compiler == "Visual Studio":
            ...
            if url == "": 
                raise Exception("Binary does not exist for these settings edit customDependenciesDownload() in OSG conan files in order to set 3rd Party url")   
